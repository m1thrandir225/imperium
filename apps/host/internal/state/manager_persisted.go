...
// PersistedStateManager implements StateManager and is persisted in a yaml file
type PersistedStateManager struct {
	state *AppState
	mu    sync.Mutex
}

// NewPersistedStateManager creates a new StateManager
func NewPersistedStateManager(appName string) (StateManager, error) {
...
func (m *PersistedStateManager) Load() error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *PersistedStateManager) Save() error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *PersistedStateManager) Get() AppState {
	m.mu.Lock()
	defer m.mu.Unlock()
	return *m.state
}
...
func (m *PersistedStateManager) Update(fn func(*AppState)) error {
