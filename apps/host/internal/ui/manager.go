...
func NewManager(stateManager state.StateManager, bus events.EventBroker) Manager {
	return newUIManager(stateManager, bus)
}

func newUIManager(stateManager state.StateManager, bus events.EventBroker) *uiManager {
	manager := &uiManager{
	...
	manager.window = manager.app.NewWindow(appWindowName)
...
func (m *uiManager) subscribeNavigation() {
...
func (m *uiManager) publish(topic string, data any) {
	if m.bus != nil {
		m.bus.Publish(topic, data)
	}
}

func (m *uiManager) getState() state.AppState {
	return m.state.Get()
}

func (m *uiManager) initializeScreens() {
...
func (m *uiManager) addScreen(screen screen) {
	m.screens[screen.Name()] = screen
}

func (m *uiManager) showScreen(name string) {
...
func (m *uiManager) onSetupSuccess() {
...
func (m *uiManager) systemTraySetup() {
	if desk, ok := m.app.(desktop.App); ok {
		if !m.shouldShowLogin() && !m.shouldShowSetup() {
			loggedInMenu := fyne.NewMenu(
				"Imperium Host",
				fyne.NewMenuItem("Open", func() {
					m.window.Show()
				}),
				fyne.NewMenuItemSeparator(),
				fyne.NewMenuItem("Logout", func() {
					m.bus.Publish(uapp.EventLogoutRequested, nil)
				}),
			)
			desk.SetSystemTrayMenu(loggedInMenu)
		} else {
			menu := fyne.NewMenu(
				"Imperium Host",
				fyne.NewMenuItem("Open", func() {
...
func (m *uiManager) onLogout() {
	m.resetScreens()
	m.showScreen(LOGIN_SCREEN)
}

func (m *uiManager) resetScreens() {
	m.screens = make(map[string]screen)
	m.initializeScreens()
}

func (m *uiManager) shouldShowLogin() bool {
	cfg := m.getState()
...
func (m *uiManager) getCurrentSession() *session.Session {
